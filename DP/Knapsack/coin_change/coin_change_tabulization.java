    ...
    public static int ks(int arr[][],int wt[],int n,int W)
    {
        for( int i=0;i<=n;i++)
        {
            for(int j=0;j<=W;j++)
            {
                ...
                {
                    arr[i][j]= arr[i][j-wt[i-1]] + arr[i-1][j];
                }
                else
                {
                    arr[i][j]=arr[i-1][j];
...
class coin_change_tabulization
{
    public static void main(String args[])
    {
        ...
        System.out.println("The Total Number Of Possible Ways to give change for Given coin Is :"+res);
    }
}
